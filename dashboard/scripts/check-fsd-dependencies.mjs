...
function isWordAt(source, index, word) {
  ...
  return (
    source.startsWith(word, index) && !/[\w$]/.test(before ?? '') && !/[\w$]/.test(after ?? '')
  );
}
...
function isEntityCrossImport(sourceRoot, source, target, targetInfo) {
  ...
  return (
    layer === 'entities' &&
    slice === targetInfo.slice &&
    marker === '@x' &&
    consumer === source.slice
  );
}
  ...
  for (const file of await sourceFiles(sourceRoot)) {
    ...
    for (const specifier of importSpecifiers(content)) {
      ...
      if (layerRank.get(targetInfo.layer) > layerRank.get(source.layer)) {
        violations.push(
          `${relative(sourceRoot, file)}: ${source.layer} cannot import ${targetInfo.layer}`,
        );
      }
      ...
      if (
        ...
        violations.push(
          `${relative(sourceRoot, file)}: ${source.layer} slices cannot import each other`,
        );
      }
    }
  }
