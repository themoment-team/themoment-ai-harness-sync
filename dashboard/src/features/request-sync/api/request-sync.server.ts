  ...
  const run = data.workflow_runs.find(
    (candidate) =>
      ...
      (candidate.status === 'queued' ||
        ...
        new Date(candidate.created_at).getTime() >= threshold),
  );
  if (!run) return null;
...
export async function requestSync(
  input: { owner: string; repo: string; userToken: string },
  ...
  const openPullRequest = await services.findOpenSyncPullRequest(
    dashboard.repository.installationId,
    input.owner,
    input.repo,
  );
  ...
  return {
    ...(await services.dispatch(dashboard.repository.fullName)),
