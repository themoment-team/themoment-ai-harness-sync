...
describe("buildSyncConfig", () => {
  ...
  it("설정 파일이 없으면 기존 자동 수신 기본값을 사용한다", () => {
    expect(parseSyncConfig(null, ["claude", "codex"]))
      .toMatchObject({ enabled: true, mode: "automatic", groups: ["claude", "codex"] });
  });

  it("빈 그룹 설정은 새 항목을 자동 수신하지 않는 고정 선택이다", () => {
    expect(parseSyncConfig("enabled: false\ngroups: []\noverrides:\n  claude/skills/api-design: true\n", ["claude"]))
      .toMatchObject({ enabled: false, mode: "fixed", groups: [] });
  });

  it("잘못된 YAML 설정은 거부한다", () => {
    expect(() => parseSyncConfig("enabled: nope", ["claude"]))
      .toThrow();
  });
});
