...
export function ConfigPreview({ owner, repo, currentYaml, selection }: ConfigPreviewProps) {
  ...
  return (
    <aside aria-label="설정 변경 미리보기">
      ...
      <p className="mt-3 text-fg-muted text-xs leading-5">PR을 만들기 전 대상 레포의 실제 sync.yml 변경 내용을 확인합니다.</p>
      <pre className="mt-4 overflow-x-auto rounded-md border border-border bg-bg-subtle p-4 font-mono text-fg text-xs leading-5">
        <code>
          {diff.map((line, index) => (
            <span key={`${line.type}-${line.value}-${index}`} className={`block min-w-max px-1 ${diffLineClassNames[line.type]}`}>
